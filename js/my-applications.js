...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const response = await fetch(`/api/applications/${cu.email}`);
    const apps = await response.json();

    // Update info with count
    const infoEl = document.querySelector('.info');
    if (infoEl) {
      infoEl.textContent = `Below is the complete history of all ${apps.length} scholarship application${apps.length === 1 ? '' : 's'} submitted by you.`;
    }
